    ...
    def __init__(self,pos_x,pos_y,color,position):
        ...
        if color == 'grey':
            ...
            for i in range(11):  # Assuming you have example0.gif to example12.gif
                if i < 10:
                    original_image = pygame.image.load(f'joy/{color}/frame_0{i}_delay-0.01s.gif')
                else:
                    original_image = pygame.image.load(f'joy/{color}/frame_{i}_delay-0.01s.gif')
                self.sprites.append(pygame.transform.scale(original_image, (original_image.get_width() // 2, original_image.get_height() // 2)))
        self.index = 0  # Index to track which sprite to display
        self.image = self.sprites[self.index]
        self.rect = self.image.get_rect(center=(pos_x, pos_y))
...
#--------------------------receive initial state of each joy---------------------------
    ...
    # Display countdown timer
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:  # Check if left mouse button is clicked
                        user_minigame0_count += 1  # Increment the count
        ...
        if minigame == 1:
            ...
            # Check user input to toggle switches
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_a:  # Blue player toggles by pressing "a"
                        player_blue_switch = 1 - player_blue_switch
                        correct_blue = (player_blue_switch == set_blue_switch)
                        draw_switches_initially()
                        draw_switch_status_circles()
                    elif event.key == pygame.K_s:  # Green player toggles by pressing "s"
                        player_green_switch = 1 - player_green_switch
                        correct_green = (player_green_switch == set_green_switch)
                        draw_switches_initially()
                        draw_switch_status_circles()
                    elif event.key == pygame.K_d:  # Yellow player toggles by pressing "d"
                        player_yellow_switch = 1 - player_yellow_switch
                        correct_yellow = (player_yellow_switch == set_yellow_switch)
                        draw_switches_initially()
                        draw_switch_status_circles()
                    elif event.key == pygame.K_f:  # Red player toggles by pressing "f"
                        player_red_switch = 1 - player_red_switch
                        correct_red = (player_red_switch == set_red_switch)
                        draw_switches_initially()
                        draw_switch_status_circles()

            # Display circle under switches
            draw_switch_status_circles()
